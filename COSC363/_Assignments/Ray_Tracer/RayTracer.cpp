...
void display()
{
	...
	float sample_sqr = NUMSAMPLES*NUMSAMPLES;
		...
		for(int j = 0; j < NUMDIV; j++)
		{
			...
			for(int ii= 0; ii< NUMSAMPLES; ii++)
			{
				...
				for(int jj=0; jj< NUMSAMPLES; jj++)
				{
					...
					glm::vec3 dir(xp+((ii+0.5)/NUMSAMPLES)*cellX, yp+((jj+0.5)/NUMSAMPLES)*cellY, -EDIST);	//direction of the primary ray
			...
			glColor3f(col.r/sample_sqr, col.g/sample_sqr, col.b/sample_sqr);
